...
export async function POST(request:Request) {
 ...
 if(body.transcript.length>25000 || typeof body.topic!=='string'||body.topic.length>2000 || typeof body.category!=='string'||typeof body.duration!=='number'||!Number.isFinite(body.duration)||body.duration<1||body.duration>600)return NextResponse.json({error:'This session could not be read. Please check your transcript and try again.'},{status:400});
