...
import { ArrowLeft, ArrowRight, Mic, Square, RotateCw, ShieldCheck, Sparkles, Play, FileText, Check, AlertCircle, Clock3, Download, Search, Users } from 'lucide-react';
...
import { requestAnalysis, requestTopic, requestTranscription, requestWordSearch } from '@/services/client';
import { demoTranscript } from '@/data/demo';
import { TranscriptUtterance, WordSearchMatch } from '@/types';
...
export default function PracticeSession({mode}:{mode:string}){
 ...
 const [state,setState]=useState<State>('ready');const [countdown,setCountdown]=useState(3);const [elapsed,setElapsed]=useState(0);const [transcript,setTranscript]=useState('');const [audioUrl,setAudioUrl]=useState('');const [error,setError]=useState('');const [notice,setNotice]=useState('');const [isDemo,setIsDemo]=useState(false);const [loadingTopic,setLoadingTopic]=useState(false);const [processingStep,setProcessingStep]=useState(0);const [transcribing,setTranscribing]=useState(false);
 const [transcriptId,setTranscriptId]=useState('');const [languageInfo,setLanguageInfo]=useState<{code:string;confidence:number}|null>(null);const [utterances,setUtterances]=useState<TranscriptUtterance[]|null>(null);
 const [searchQuery,setSearchQuery]=useState('');const [searchResults,setSearchResults]=useState<WordSearchMatch[]|null>(null);const [searching,setSearching]=useState(false);const [searchError,setSearchError]=useState('');
 ...
 setTranscriptId('');setLanguageInfo(null);setUtterances(null);setSearchQuery('');setSearchResults(null);setSearchError('');
 setState('countdown');
 ...
 if(!audio?.size||!mounted.current){if(mounted.current)setNotice('No audio was captured. You can still review your transcript, or record another attempt.');return;}
 setAudioUrl(URL.createObjectURL(audio));
 setTranscribing(true);setNotice('Transcribing your recording with AssemblyAI…');
 try{
 const result=await requestTranscription(audio,side?`${side}: ${topic}`:topic);
 if(!mounted.current)return;
 setTranscriptId(result.id);
 setLanguageInfo(result.language_code?{code:result.language_code,confidence:result.language_confidence??0}:null);
 setUtterances(result.utterances);
 if(result.text.trim()){setTranscript(result.text.trim());setNotice('');}
 else setNotice('AssemblyAI could not detect speech in this recording. You can type your transcript below.');
 }catch(e){
 if(mounted.current)setNotice(e instanceof Error?e.message:'Automatic transcription is unavailable. Review or edit the transcript below before analyzing.');
 }finally{if(mounted.current)setTranscribing(false);}
 }catch{setError('The audio recording could not be finalized. Your transcript is still available—review it below or try recording again.');}
 }
 finishRef.current=()=>{void finish();};
 function cancelCountdown(){run.current++;recorder.current?.dispose();setState('ready');}
 function useDemo(){setTopic(SAMPLE_TOPIC);setSide(null);setCategoryId('everyday-life');setTranscript(demoTranscript);setIsDemo(true);setElapsed(60);elapsedRef.current=60;setAudioUrl('');setError('');setNotice('This is a sample transcript, not a recording of your voice. Demo sessions are clearly labeled in your history.');setTranscriptId('');setLanguageInfo(null);setUtterances(null);setSearchQuery('');setSearchResults(null);setSearchError('');setState('review');}
 function manual(){setIsDemo(false);setElapsed(60);elapsedRef.current=60;setError('');setTranscript('');setAudioUrl('');setNotice('Paste what you said or transcribe your own recording. Feedback is based on this text; 60 seconds is used as the estimated speaking duration.');setTranscriptId('');setLanguageInfo(null);setUtterances(null);setSearchQuery('');setSearchResults(null);setSearchError('');setState('review');}
 async function searchWord(){
 if(!transcriptId||searching)return;const words=searchQuery.split(',').map(w=>w.trim()).filter(Boolean);
 if(!words.length){setSearchError('Enter at least one word or phrase to search for.');return;}
 setSearching(true);setSearchError('');setSearchResults(null);
 try{const {matches}=await requestWordSearch(transcriptId,words);if(mounted.current)setSearchResults(matches);}
 catch(e){if(mounted.current)setSearchError(e instanceof Error?e.message:'Search failed. Please try again.');}
 ...
 <div className="practice-bottom"><ShieldCheck size={13}/><span>Your recording is sent to AssemblyAI to generate a transcript; analysis then runs on that text. English supported.</span></div></section>
 ...
 {state==='review'&&<section className="panel transcript-editor"><div className="panel-heading"><div><h2>{isDemo?'Explore the sample transcript':'Review your transcript'}</h2><p>{isDemo?'A sample 60-second response to show you the full experience.':'Correct any transcription errors before getting your feedback.'}</p></div><span className="subtle-tag">{elapsed}s · {transcript.trim()?transcript.trim().split(/\s+/).length:0} words{languageInfo&&` · ${languageInfo.code.toUpperCase()} (${Math.round(languageInfo.confidence*100)}% confidence)`}</span></div>{audioUrl&&<><audio controls src={audioUrl} aria-label="Play your speaking recording"/><a href={audioUrl} download="vocalis-practice.webm" className="text-link" style={{fontSize:10,marginBottom:13}}><Download size={12}/>Save your recording</a></>}
 {utterances&&utterances.length>1&&<div className="demo-banner" role="status"><Users size={13} style={{flexShrink:0}}/><span>AssemblyAI detected {new Set(utterances.map(u=>u.speaker)).size} speakers in this recording: {utterances.map((u,i)=><span key={i}><strong>Speaker {u.speaker}</strong>: {u.text} </span>)}</span></div>}
 <label htmlFor="session-transcript" className="sr-only">Session transcript</label><textarea id="session-transcript" value={transcript} maxLength={25000} onChange={e=>setTranscript(e.target.value)} placeholder="Your transcript goes here. If live transcription wasn’t available, listen to your recording and add what you said."/><p className="editor-note">Feedback uses a local text-based coaching model unless an external provider is configured. Voice qualities and pauses are not measured from this transcript.</p>
 {transcriptId&&<div className="word-search"><label htmlFor="word-search-input" className="sr-only">Search your transcript</label><div className="word-search-row"><Search size={13} style={{flexShrink:0}}/><input id="word-search-input" type="text" value={searchQuery} onChange={e=>setSearchQuery(e.target.value)} placeholder="Search for a word or phrase (comma-separate for several)" onKeyDown={e=>{if(e.key==='Enter'){e.preventDefault();void searchWord();}}}/><button className="text-button" type="button" disabled={searching} onClick={()=>void searchWord()}>{searching?'Searching…':'Search'}</button></div>{searchError&&<p className="editor-note" role="alert">{searchError}</p>}{searchResults&&(searchResults.length?<ul className="word-search-results">{searchResults.map(m=><li key={m.text}>“{m.text}” — {m.count} {m.count===1?'time':'times'}</li>)}</ul>:<p className="editor-note">No matches found in your transcript.</p>)}</div>}
 <div className="editor-actions"><button className="text-button" onClick={()=>{setState('ready');setNotice('');setError('');setIsDemo(false);setAudioUrl('');}}><RotateCw size={12} style={{display:'inline',marginRight:5}}/>Record again</button><button className="button button-primary" onClick={analyze} disabled={transcribing||transcript.trim().split(/\s+/).length<5}>{transcribing?'Transcribing…':'Analyze my response'} <Sparkles size={15}/></button></div></section>}
