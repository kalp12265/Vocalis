  ...
  start(onText: (text: string) => void, onError: (message: string) => void) {
    ...
    recognition.onerror = (event) => {
      if (event.error !== "aborted")
        onError(
          "Live transcription is unavailable. Your audio is still recording; you can add or correct the transcript afterward.",
        );
    };
...
export class ManualTranscriptionService implements TranscriptionService {
  start() {
    return false;
  }
  stop() {}
...
export function createTranscriptionService(): TranscriptionService {
  return process.env.NEXT_PUBLIC_STT_PROVIDER === "manual"
    ? new ManualTranscriptionService()
    : new BrowserTranscriptionService();
}
